...
/* ---------- YAML Parser Check ---------- */
function waitForYAML() {
    return new Promise((resolve) => {
        const checkYAML = () => {
            if (typeof YAML !== 'undefined') {
                console.log('YAML library is now available');
                resolve();
            } else {
                console.log('Waiting for YAML library...');
                setTimeout(checkYAML, 200);
            }
        };
        checkYAML();
    });
...
function parseYAMLorJSON(text) {
    ...
    if (typeof YAML !== 'undefined') {
        try {
            ...
            console.warn('YAML parsing failed, trying JSON:', yamlError.message);
        }
    } else {
        console.warn('YAML not available, trying simple YAML parser');
    }

    // Simple YAML parser for arrays
    try {
        // Check if it's a simple YAML array (our case)
    ...
    try {
        ...
        throw new Error(`Failed to parse file as YAML or JSON: ${jsonError.message}`);
    }
}

/* ---------- Initialization ---------- */
async function init() {
    // Don't wait for YAML - use our fallback parser
    console.log('Initializing with fallback YAML parser...');
...
function renderAds(ads, type) {
    ...
    // Show all ads instead of just one
    ads.forEach((ad, index) => {
        const adCard = document.createElement('div');
        adCard.className = 'ad-card';

        if (type === 'tiktok') {
            // Handle TikTok ads as simple text (since we changed the generator)
            const adText = typeof ad === 'string' ? ad : JSON.stringify(ad);
            adCard.innerHTML = `
                <div class="ad-header">
                ...
                <div class="ad-actions">
                    <button class="action-btn" onclick="copyToClipboard('${encodeURIComponent(adText)}')">📋 نسخ</button>
                </div>
            `;
        } else {
            ...
            adCard.innerHTML = `
                ...
                <div class="ad-actions">
                    <button class="action-btn" onclick="copyToClipboard('${encodeURIComponent(ad)}')">📋 نسخ</button>
                </div>
            `;
        }
